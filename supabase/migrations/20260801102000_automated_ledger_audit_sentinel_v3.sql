-- Migration: Automated Ledger Audit Sentinel & Daily Reconciler v3
...
BEGIN
  ...
  FOR v_rec IN SELECT agent_id, balance FROM public.wallets LOOP
    ...
    IF v_has_credit_txs THEN
      EXECUTE 'SELECT COALESCE(SUM(amount), 0) FROM public.credit_transactions WHERE (agent_id = $1 OR user_id = $1) AND status IN (''success'', ''paid'', ''completed'')'
