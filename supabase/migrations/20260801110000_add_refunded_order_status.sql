-- Migration: Support 'refunded' status for refunded orders
-- Updates refund_failed_order to set status = 'refunded' and updates existing refunded orders
...
-- Update all existing auto-refunded orders to status = 'refunded'
UPDATE public.orders
SET status = 'refunded', updated_at = now()
WHERE auto_refunded = true AND status = 'fulfillment_failed';
