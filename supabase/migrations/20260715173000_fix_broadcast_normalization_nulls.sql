-- Migration: Fix Broadcast Normalization Nulls
-- Description: Redefines public.broadcast_push_notification to filter out profiles where public.normalize_phone_sql(phone) returns NULL. This prevents pg_cron jobs from failing due to NOT NULL constraint violations in public.sms_logs.recipient.
...
BEGIN
  ...
  -- Aggregate normalized phone numbers as a comma-separated string (filtering out NULLs)
  SELECT COALESCE(string_agg(public.normalize_phone_sql(phone), ','), '') INTO v_phone_string
  FROM public.profiles
  WHERE (is_agent = true OR sub_agent_approved = true) 
    AND phone IS NOT NULL 
    AND phone != ''
    AND public.normalize_phone_sql(phone) IS NOT NULL;
  ...
  IF v_phone_string IS NOT NULL AND v_phone_string != '' AND v_sms_api_key IS NOT NULL AND v_sms_api_key != '' THEN
    PERFORM net.http_post(
      ...
      body    := jsonb_build_object(
        'to', v_phone_string, -- Comma-separated string of phone numbers
