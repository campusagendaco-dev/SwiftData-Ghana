-- Migration: New Beneficiary Warning SMS Notification
-- Description: Sends an SMS to users who bypassed beneficiary validation with sender ID 'swiftupdate', warning them of the 24-72 hour delivery window and inviting them to the WhatsApp update channel.
...
DROP TRIGGER IF EXISTS trg_new_beneficiary_order_sms ON public.orders;
CREATE TRIGGER trg_new_beneficiary_order_sms
  AFTER INSERT ON public.orders
  FOR EACH ROW
  EXECUTE FUNCTION public.handle_new_beneficiary_order_trigger();
