...
serve(async (req) => {
  ...
  try {
          ...
          if (!network || !packageSize || !customerPhone) {
            ...
            if (result.ok) {
              ...
              if (order.customer_phone) {
                await sendPaymentSms(supabase, order.customer_phone, "order_failed", {
                  package: order.package_size || "Data",
    ...
    if (orderType === "wallet_topup" && resolvedAgentId) {
      await sendWalletTopupSms(supabase, resolvedAgentId, verifiedAmount);
    }
