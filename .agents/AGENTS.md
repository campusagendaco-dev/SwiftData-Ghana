...
* **Server-Side Purchase Verification**: NEVER trust price or amount values passed from client-side payloads when processing purchases. ALWAYS calculate, resolve, and verify package prices strictly on the server-side (in database RPCs and Edge Functions) against global package settings (`global_package_settings`) or agent pricing tables before allowing any order to proceed or debiting wallets.
