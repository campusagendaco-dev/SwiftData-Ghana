...
export default function DashboardRefundedOrders() {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 max-w-6xl space-y-6 sm:space-y-8 animate-in fade-in duration-300">
      {/* Hero Banner Header */}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        <div className={cn("p-4 sm:p-6 rounded-2xl sm:rounded-3xl border transition-all duration-300", isDark ? "bg-card/70 border-purple-500/20 shadow-xl shadow-purple-950/10" : "bg-white border-purple-100 shadow-md")}>
          <div className="flex items-center justify-between">
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-muted-foreground">Total Returned</span>
          ...
          <div className="mt-2 sm:mt-3 flex items-baseline gap-2">
            <span className="text-xl sm:text-3xl font-black tracking-tight text-purple-600 dark:text-purple-400">
              GH₵ {totalRefundedAmount.toFixed(2)}
            </span>
          </div>
          <p className="text-[10px] sm:text-xs text-muted-foreground mt-1">100% credited to wallet</p>
        </div>
        ...
        <div className={cn("p-4 sm:p-6 rounded-2xl sm:rounded-3xl border transition-all duration-300", isDark ? "bg-card/70 border-blue-500/20 shadow-xl shadow-blue-950/10" : "bg-white border-blue-100 shadow-md")}>
          <div className="flex items-center justify-between">
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-muted-foreground">Refunded</span>
          ...
          <div className="mt-2 sm:mt-3">
            <span className="text-xl sm:text-3xl font-black tracking-tight text-blue-600 dark:text-blue-400">
              {totalRefundedCount} Orders
            </span>
          </div>
          <p className="text-[10px] sm:text-xs text-muted-foreground mt-1">Auto-refunded orders</p>
        </div>

        <div className={cn("col-span-2 lg:col-span-1 p-4 sm:p-6 rounded-2xl sm:rounded-3xl border transition-all duration-300", isDark ? "bg-card/70 border-emerald-500/20 shadow-xl shadow-emerald-950/10" : "bg-white border-emerald-100 shadow-md")}>
          <div className="flex items-center justify-between">
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-muted-foreground">Guarantee</span>
          ...
          <div className="mt-2 sm:mt-3">
            <span className="text-lg sm:text-2xl font-extrabold tracking-tight text-emerald-600 dark:text-emerald-400">
              Instant Auto-Credit
            </span>
          </div>
          <p className="text-[10px] sm:text-xs text-muted-foreground mt-1">Zero financial risk on failed orders</p>
        </div>
      </div>
      ...
      <div className={cn("rounded-2xl sm:rounded-3xl border overflow-hidden transition-all backdrop-blur-xl shadow-xl", isDark ? "bg-card/70 border-border" : "bg-white border-gray-200")}>
        {loading ? (
          ...
          <div className="p-12 sm:p-16 text-center space-y-4">
            ...
            <h3 className="text-base sm:text-lg font-bold">No Refunded Orders Found</h3>
                ...
                return (
                  <div key={o.id} className="p-4 space-y-3">
                    ...
                    <div className="flex items-center justify-between text-xs pt-1">
                      <div>
                        <div className="font-bold text-foreground">{o.network} {o.package_size}</div>
                        <div className="font-mono text-muted-foreground mt-0.5">{o.customer_phone || "—"}</div>
                      </div>
                      <div className="text-right">
                        <div className="font-black text-base text-purple-600 dark:text-purple-400">+GH₵ {amount}</div>
                        <div className="text-[10px] text-muted-foreground">Returned to Wallet</div>
                      </div>
                    </div>

                    <div className="text-[11px] text-muted-foreground bg-muted/30 p-2 rounded-xl truncate" title={o.failure_reason || o.refund_reason || "Auto-refund"}>
                    ...
                    <div className="flex items-center justify-between pt-1 gap-2">
                      ...
                      <Button
                        variant="default"
                        size="sm"
                        className="h-9 px-4 rounded-xl text-xs font-bold gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-white shadow-md"
                        onClick={() => handleVerifyAndRetry(o)}
                        disabled={isVerifying}
                      >
                        {isVerifying ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Play className="w-3.5 h-3.5" />}
                        {isBeneficiaryError ? "Check & Retry" : "Verify & Retry"}
                      </Button>
                    </div>
                  </div>
                );
